...
# Piden nombres declarativos y buenas practicas pero la variable q no es muy declarativa que digamos...
# también se podría haber utilizado un bucle for
def FindQueue():
    q = hiQueue
    while q > 0:
        if len(queue[q]) > 0:
...
quantum = {} #para generar la lista de quantum de cada proceso
if options.quantumList != '':
    # en este caso significa que le pasamos una lista que asigna los quantums a cada cola
    # instead, extract number of queues and their time slic
    quantumLengths = options.quantumList.split(',') # desarmamos el string dado desde terminal y lo convertimos en una lista
    numQueues = len(quantumLengths) # y ahora asignamos el numero de colas segun la cantidad de quantums dados
    qc = numQueues - 1 # QUE SIGNIFICA QC? -> lo vamos a usar para dar vuelta el orden de los quantums
    # ESTO SE PODIA HACER SIMPLEMENTE DANDO VUELTA LA LISTA USANDO REVERSE?
    for i in range(numQueues):
        quantum[qc] = int(quantumLengths[i]) # esto va a asignar los quantums a la lista original. Sucede que luego del split queda en el indice mas bajo de quantumLengths y aquí
        qc -= 1                                # queremos que se alinee con el número de prioridad de la cola, en este caso en quantumLenghts[0] sería el quantum de la cola de mayor prioridad, pero lo mandamos a quantums[el_numero_mas_alto_posible]
else:
    for i in range(numQueues):
        quantum[i] = int(options.quantum) # Si no utilizamos la opcion de -Q y asignamos quantum con -q, se encarga de poner a todas las colas el mismo valor (aquel que pasamos)

allotment = {} #ni idea que es allotment (SUPONGO QUE SERÁ LA CANTIDAD DE TAREAS QUE PUEDE HABER EN CADA COLA YA QUE LA CANTIDAD DEFAULT ES 1)
...
jobCnt = 0
if options.jlist != '':
    # Si efectivamente se pasa una lista de trabajos como parte de las opciones
    # vamos a darle formato al string pasado como lista de trajos
    allJobs = options.jlist.split(':') # aquí obtendremos una lista en la cual cada indice tiene toda la información de un trabajo en particular
    for j in allJobs:
        # en este bucle se extrae la información relevante de cada trabajo
        jobInfo = j.split(',') # y aquí sus tres atributos
        if len(jobInfo) != 3:
            # Error handling
        ...
        # Declaramos tres nuevas variables que almacenaran nuestros datos
        ...
        job[jobCnt] = {'currPri':hiQueue, 'ticksLeft':quantum[hiQueue],
                       ...
                       'firstRun':-1, 'waitingTime':0} # AGREGAMOS WAITING TIME
...
while finishedJobs < totalJobs:
    ...
    if options.boost > 0 and currTime != 0:
        if currTime % options.boost == 0:
            ...
            for q in range(numQueues-1):
                for j in queue[q]:
                    if job[j]['doingIO'] == False: # que pasa si esta haciendo io?
    ...
    throughput_print_time = 2
    if currTime != 0 and currTime % throughput_print_time == 0:
        currThroughput = finishedJobs / currTime
        #print('[ time %d ] CURRENT THROUGHPUT: %.2f' % (currTime, currThroughput)) # formato limitado a 2 decimales para disminuir el ruido en los datos
    ...
    currQueue = FindQueue()
    if currQueue == -1:
        # esto sucede si no hay ninguna tarea para realizar
    ...
    # =======================================================
    for i in range(numJobs):
        finished = job[i]['timeLeft'] == 0
        started = currTime >= job[i]['startTime']

        #print(i != currJob, started, not finished)
        if i != currJob and not job[i]['doingIO'] and started and not finished:
            job[i]['waitingTime'] += 1
        #print('job %d waitingtime %d' % (i, job[i]['waitingTime']))
    # =======================================================
...
turnaroundSum = 0
# ======================
waitingTimeSum = 0 # agregamos el acumulador para la suma de los tiempos de espera
# ======================
for i in range(numJobs):
    response   = job[i]['firstRun'] - job[i]['startTime']
    turnaround = job[i]['endTime'] - job[i]['startTime']
    waitingTime = job[i]['waitingTime']
...
print('\n  Avg %2d: startTime n/a - response %.2f - turnaround %.2f - waitingTime %.2f' % 
      (i, float(responseSum)/numJobs, float(turnaroundSum)/numJobs, float(waitingTimeSum)/numJobs))
print('\n')
